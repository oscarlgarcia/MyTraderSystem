...
def test_feed_support_matrix_reflects_current_live_scope():
    ...
    assert FEED_SUPPORT_MATRIX["kline"].recovery_capability == "approximate"
    assert FEED_SUPPORT_MATRIX["kline"].supports_exact_recovery is False
    assert FEED_SUPPORT_MATRIX["kline"].supports_exact_verified_recovery is False
...
def test_validate_live_feed_support_requires_exact_recovery_and_handoff():
    ...
    with pytest.raises(ValueError, match="kline does not support exact recovery"):
        validate_live_feed_support(("kline",), require_exact_recovery=True, require_handoff=True)
