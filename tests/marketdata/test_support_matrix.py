...
def test_feed_support_matrix_reflects_current_live_scope():
    ...
    assert FEED_SUPPORT_MATRIX["trade"].live_validation_basis == "runtime_validated"
    assert FEED_SUPPORT_MATRIX["trade"].recovery_capability == "exact_verified"
    assert FEED_SUPPORT_MATRIX["trade"].supports_exact_recovery is True
    ...
    assert FEED_SUPPORT_MATRIX["kline"].live_validation_basis == "runtime_validated"
    ...
    assert FEED_SUPPORT_MATRIX["book"].operational_tier == "excluded"
...
def test_validate_paper_feed_support_matches_declared_scope():
    ...
    assert replay_validated_paper_feed_types() == ("trade",)
    assert runtime_validated_paper_feed_types() == ("kline",)
    assert live_supported_feed_types() == ("trade", "kline")
    assert runtime_validated_live_feed_types() == ("trade", "kline")
    assert excluded_feed_types() == ("book",)
...
def test_validate_live_feed_support_requires_exact_recovery_and_handoff():
    assert validate_live_feed_support(("trade",), require_exact_recovery=False, require_handoff=True) == ("trade",)
    assert validate_live_feed_support(("trade",), require_exact_recovery=True, require_handoff=True) == ("trade",)
